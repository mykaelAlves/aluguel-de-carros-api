...
use crate::{
    AppState,
    entities::{
        Produto, ProdutoFiltro, Role, Usuario, UsuarioLogin, UsuarioRegistro,
    },
};
...
pub async fn login(
    ...
    let rcv_hashed_pwd =
        hex::encode(sha2::Sha256::digest(usuario_login.password.as_bytes()));
    ...
    state
        ...
        .iter()
        .find(|u| {
            u.email == usuario_login.email && u.pwd_hash == rcv_hashed_pwd
        })
...
pub async fn register(
    State(state): State<RwAppState>,
    Json(usuario_registro): Json<UsuarioRegistro>,
) -> Result<(StatusCode, Token), StatusCode> {
    if state
        .read()
        .await
    ...
    state.write().await.usuarios.push(Usuario {
        ...
        pwd_hash: hex::encode(sha2::Sha256::digest(
            usuario_registro.password.as_bytes(),
        )),
        role: Role::Cliente,
    });
...
pub async fn produto_update(
    State(state): State<RwAppState>,
    Path(sku): Path<String>,
    Json(produto_atualizado): Json<Produto>,
) -> impl IntoResponse {
    let mut guard = state.write().await;
